...
def train(base_loader, val_loader,  model, start_epoch, stop_epoch, params, val_loader2=None, labeled_target_loader=None):
  ...
  if(params.stage=='pretrain'):
      ...
      optimizer = torch.optim.Adam([{'params': model.feature.parameters()},
                                {'params': model.disentangle_model.parameters()},
                                {'params': model.attention_sp.parameters()},
                                {'params': model.attention_ir.parameters()},
  ...
  for epoch in range(start_epoch, stop_epoch):
    ...
    if (params.stage == 'pretrain'):
      ...
      total_it = model.train_loop(epoch, base_loader, labeled_target_loader, optimizer,
                                  total_it)  # model are called by reference, no need to return
    model.eval()

    acc = model.test_loop(val_loader)
    if val_loader2 != None:
      acc2 = model.test_loop2(val_loader2)
    if acc > max_acc:
    ...
    if val_loader2 != None:
      if acc2 > max_acc2:
        print("target best model! save...")
        max_acc2 = acc2
        outfile = os.path.join(params.checkpoint_dir, 'target_best_model.tar')
        torch.save({'epoch': epoch, 'state': model.state_dict()}, outfile)
      else:
        print("GG! best accuracy {:f}".format(max_acc2))
...
if __name__ == '__main__':
  ...
  source_val_file2 = os.path.join(params.data_dir, params.target_set, 'val.json')
  ...
  if params.stage == 'pretrain':
    ...
    val_loader = val_datamgr.get_data_loader(source_val_file, aug=False)
    if os.path.exists(source_val_file2):
      val_loader2 = val_datamgr.get_data_loader(source_val_file2, aug=False)
    else:
      val_loader2 = None
    ...
    labeled_base_file_dict['EuroSAT'] = 'sources/labled_base_EuroSAT_' + str(params.target_num_label) + '.json'
    labeled_base_file_dict['CropDisease'] = 'sources/labled_base_CropDisease_' + str(params.target_num_label) + '.json'
    labeled_base_file_dict['ISIC'] = 'sources/labled_base_ISIC_' + str(params.target_num_label) + '.json'
    labeled_base_file_dict['chestX'] = 'sources/labled_base_chestX_' + str(params.target_num_label) + '.json'
  ...
  if (params.stage == 'pretrain'):
    ...
    model = train(base_loader, val_loader, model, start_epoch, stop_epoch, params, val_loader2, labeled_target_loader)